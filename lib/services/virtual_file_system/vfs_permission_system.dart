...
class VfsPermissionMask {
  ...
  bool get isSystemProtected =>
      (systemFlags & VfsPermission.systemProtected) != 0;
  ...
  factory VfsPermissionMask.fromJson(Map<String, dynamic> json) {
    return VfsPermissionMask(
      ...
      groupPermissions:
          json['groupPermissions'] as int? ?? VfsPermission.readOnly,
      otherPermissions:
          json['otherPermissions'] as int? ?? VfsPermission.readOnly,
...
class VfsPermissionManager {
  static final VfsPermissionManager _instance =
    ...
    try {
      ...
      if (metadata.containsKey(permissionKey)) {
        final permissionData =
            jsonDecode(metadata[permissionKey]!) as Map<String, dynamic>;
  ...
  Future<bool> canWrite(String path) async {
    ...
    if (hasStoredPermissions) {
      ...
      try {
        ...
        final canWrite = parentPermissions.hasPermission(
          VfsPermissionType.user,
          VfsPermission.write,
        );
        debugPrint('DEBUG canWrite: Parent write permission = $canWrite');
        return canWrite;
      } catch (e) {
        // 如果父目录也不存在，使用默认权限（允许）
        debugPrint(
  ...
  bool _isProtectedPath(VfsPath vfsPath) {
    ...
    if (vfsPath.database == 'r6box' && vfsPath.collection == 'fs') {
      for (final protectedPath in _protectedPaths) {
        if (vfsPath.path == protectedPath ||
            vfsPath.path.startsWith('$protectedPath/')) {
          return true;
        }
      }
    }
