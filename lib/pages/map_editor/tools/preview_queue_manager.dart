...
class StickyNotePreviewQueueItem {
  ...
  StickyNotePreviewQueueItem copyWith({
    ...
    return StickyNotePreviewQueueItem(
      ...
      createdAt: createdAt ?? this.createdAt,
      isWaitingForStickyNote: isWaitingForStickyNote ?? this.isWaitingForStickyNote,
    );
  }
}
...
class PreviewQueueManager {
  ...
  // 预览队列变化通知器
  final ValueNotifier<List<PreviewQueueItem>> _queueNotifier = 
      ValueNotifier([]);
  
  // 便签预览队列变化通知器
  final ValueNotifier<List<StickyNotePreviewQueueItem>> _stickyNoteQueueNotifier = 
      ValueNotifier([]);
  ...
  }) : _collaborationManager = collaborationManager ?? _getGlobalCollaborationStateManager();
  ...
  /// 将预览项转换为绘制元素
  MapDrawingElement convertToDrawingElement(
    ...
    for (final entry in _layerQueues.entries) {
      ...
      if (tryLockLayer(layerId)) {
        ...
        for (final item in itemsToProcess) {
          ...
          print('[PreviewQueueManager] 队列中的预览已处理: ${item.id}, z值: $zIndex (基于实时图层状态)');
        }
  ...
  void processLayerWaitingPreviews(String layerId) {
    ...
    if (tryLockLayer(layerId)) {
      ...
      for (final item in itemsToProcess) {
        ...
        print('[PreviewQueueManager] 图层 $layerId 队列中的预览已处理: ${item.id}, z值: $zIndex');
      }
  ...
  List<PreviewQueueItem> getWaitingItems() {
    ...
    for (final layerQueue in _layerQueues.values) {
      waitingItems.addAll(
        layerQueue.where((item) => item.isWaitingForLayer)
      );
    }
    return waitingItems;
  }
  ...
  List<String> getLayersWithQueue() {
    return _layerQueues.keys.where((layerId) => 
      _layerQueues[layerId]?.isNotEmpty == true
    ).toList();
  }
  ...
  /// 获取便签预览队列通知器
  ValueNotifier<List<StickyNotePreviewQueueItem>> get stickyNoteQueueNotifier => _stickyNoteQueueNotifier;
  ...
  void addStickyNotePreviewToQueue(
    ...
    if (isLocked) {
      ...
      print('[PreviewQueueManager] 便签 $stickyNoteId 被锁定，预览已加入队列，当前队列长度: $queueLength');
    } else {
      // 便签未锁定，尝试锁定并立即处理
      if (tryLockStickyNote(stickyNoteId)) {
        ...
        _stickyNoteQueues.putIfAbsent(stickyNoteId, () => []).add(queueItem);
        final queueLength = _stickyNoteQueues[stickyNoteId]!.length;
        print('[PreviewQueueManager] 无法锁定便签 $stickyNoteId，预览已加入队列，当前队列长度: $queueLength');
      }
    }
  ...
  void processStickyNoteWaitingPreviews(String stickyNoteId) {
    ...
    if (tryLockStickyNote(stickyNoteId)) {
      ...
      // 处理该便签的所有预览项
      final itemsToProcess = List<StickyNotePreviewQueueItem>.from(stickyNoteQueue);
      ...
      for (int i = 0; i < itemsToProcess.length; i++) {
        ...
        print('[PreviewQueueManager] 便签 $stickyNoteId 队列项目 ${i + 1}/$totalItems 已处理: ${item.id}');
      }
      
      // 清空该便签的队列
      stickyNoteQueue.clear();
      _stickyNoteQueues.remove(stickyNoteId);
      print('[PreviewQueueManager] 便签 $stickyNoteId 队列已清空，所有 $totalItems 个项目处理完成');
  ...
  void _processStickyNotePreviewItem(StickyNotePreviewQueueItem item) {
    // 实时计算便签元素的z值，确保不会冲突
    final currentMaxZIndex = onGetStickyNoteMaxZIndex?.call(item.stickyNote.id) ?? 0;
    final elementWithUpdatedZIndex = item.element.copyWith(
      zIndex: currentMaxZIndex + 1,
    );
    
    // 添加更新了z值的元素到便签
    final updatedStickyNote = item.stickyNote.addElement(elementWithUpdatedZIndex);
  ...
  List<String> getStickyNotesWithQueue() {
    return _stickyNoteQueues.keys.where((stickyNoteId) => 
      _stickyNoteQueues[stickyNoteId]?.isNotEmpty == true
    ).toList();
  }
