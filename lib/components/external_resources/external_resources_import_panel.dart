...
class ExternalResourcesImportPanel extends StatefulWidget {
  ...
  @override
  State<ExternalResourcesImportPanel> createState() =>
      _ExternalResourcesImportPanelState();
}

class _ExternalResourcesImportPanelState
    extends State<ExternalResourcesImportPanel> {
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text('选择性导入', style: theme.textTheme.titleMedium),
                    ],
                  ),
                  const SizedBox(height: 8),
                  const Text('从JSON文件选择性导入地图、传奇和本地化数据。支持版本比较、冲突处理和重命名导入。'),
                ],
              ),
            ),
          ),
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('导入选项', style: theme.textTheme.titleMedium),
                    const SizedBox(height: 16),
                    ...
                    CheckboxListTile(
                      ...
                      onChanged: _preview!.hasLocalizations
                          ? (value) {
                              setState(() {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('地图数据详情', style: theme.textTheme.titleMedium),
                        const Spacer(),
                        Text(
                          '已选择 ${_selectedMapIds.length}/${_importMapItems.length}',
                        ),
                        const SizedBox(width: 16),
                        TextButton(
                          onPressed: () {
                            setState(() {
                              if (_selectedMapIds.length ==
                                  _importMapItems.length) {
                                _selectedMapIds.clear();
                              } else {
                                _selectedMapIds = _importMapItems
                                    .map((e) => e.id)
                                    .toSet();
                              }
                            });
                          ...
                          child: Text(
                            _selectedMapIds.length == _importMapItems.length
                                ? '全不选'
                                : '全选',
                          ),
                        ),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('传奇数据详情', style: theme.textTheme.titleMedium),
                        const Spacer(),
                        Text(
                          '已选择 ${_selectedLegendIds.length}/${_importLegendItems.length}',
                        ),
                        const SizedBox(width: 16),
                        TextButton(
                          onPressed: () {
                            setState(() {
                              if (_selectedLegendIds.length ==
                                  _importLegendItems.length) {
                                _selectedLegendIds.clear();
                              } else {
                                _selectedLegendIds = _importLegendItems
                                    .map((e) => e.id)
                                    .toSet();
                              }
                            });
  ...
  bool _canImport() {
    return !_isImporting && _preview != null && _preview!.hasAnyData;
  }
  ...
  Future<void> _analyzeFile(String filePath) async {
    try {
      ...
      final jsonData =
          jsonDecode(content) as Map<String, dynamic>; // 创建详细的导入项目列表
  ...
  Future<void> _createDetailedImportItems(Map<String, dynamic> jsonData) async {
    ...
    if (jsonData.containsKey('maps')) {
      ...
      if (mapsData is List) {
        // 直接的数组格式
        maps = mapsData;
      } else if (mapsData is Map<String, dynamic> &&
          mapsData.containsKey('data')) {
        // 嵌套格式：maps.data
    ...
    if (jsonData.containsKey('legends')) {
      ...
      if (legendsData is List) {
        // 直接的数组格式
        legends = legendsData;
      } else if (legendsData is Map<String, dynamic> &&
          legendsData.containsKey('data')) {
        // 嵌套格式：legends.data
  ...
  Future<ImportMapItem?> _createMapItem(Map<String, dynamic> mapData) async {
    try {
      ...
      final existingMap = existingMaps
  ...
  Future<ImportLegendItem?> _createLegendItem(
    Map<String, dynamic> legendData,
  ) async {
    try {
      ...
      final existingLegend = existingLegends
    ...
    return PopupMenuButton<ImportAction>(
      ...
      itemBuilder: (context) => [
        const PopupMenuItem(
          ...
          child: Row(
            children: [Icon(Icons.swap_horiz), SizedBox(width: 8), Text('替换')],
          ),
        ),
        const PopupMenuItem(
          value: ImportAction.skip,
          child: Row(
            children: [Icon(Icons.skip_next), SizedBox(width: 8), Text('跳过')],
          ),
        ),
        const PopupMenuItem(
    ...
    try {
      ...
      if (_importMaps && _selectedMapIds.isNotEmpty) {
        for (final mapItem in _importMapItems) {
          if (_selectedMapIds.contains(mapItem.id) &&
              mapItem.importAction != ImportAction.skip) {
      ...
      if (_importLegends && _selectedLegendIds.isNotEmpty) {
        for (final legendItem in _importLegendItems) {
          if (_selectedLegendIds.contains(legendItem.id) &&
              legendItem.importAction != ImportAction.skip) {
  ...
  Future<void> _importSingleMap(ImportMapItem mapItem) async {
    try {
      ...
      if (mapData.containsKey('legendGroups') &&
          mapData['legendGroups'] is List) {
      ...
      if (mapData.containsKey('imageData')) {
        ...
        if (imageDataJson is String && imageDataJson.isNotEmpty) {
          try {
            ...
            print(
              'Warning: Failed to decode imageData for map ${mapItem.title}: $e',
            );
          }
        }
      }
  ...
  Future<void> _importSingleLegend(ImportLegendItem legendItem) async {
    try {
      ...
      if (legendItem.conflictStatus != ConflictStatus.noConflict &&
        ...
        final existingLegends = await _legendService.getAllLegends();
        final existingLegend = existingLegends
            .where((l) => l.title == legendItem.title)
            .firstOrNull;
        if (existingLegend != null) {
      ...
      if (legendData.containsKey('imageData')) {
        ...
        if (imageDataJson is String && imageDataJson.isNotEmpty) {
          try {
            ...
            print(
              'Warning: Failed to decode imageData for legend ${legendItem.title}: $e',
            );
          }
        }
      }
  ...
  void _handleImportAction(
    ...
    setState(() {
      if (isMap) {
        final itemIndex = _importMapItems.indexWhere(
          (item) => item.id == itemId,
        );
        if (itemIndex != -1) {
        ...
        final itemIndex = _importLegendItems.indexWhere(
          (item) => item.id == itemId,
        );
        if (itemIndex != -1) {
