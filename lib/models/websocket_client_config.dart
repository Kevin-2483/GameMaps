...
class ServerConfig {
  ...
  ServerConfig copyWith({String? host, int? port}) {
    return ServerConfig(host: host ?? this.host, port: port ?? this.port);
  }
}
...
class ClientKeyConfig {
  ...
  const ClientKeyConfig({required this.publicKey, required this.privateKeyId});
  ...
  ClientKeyConfig copyWith({String? publicKey, String? privateKeyId}) {
...
class WebApiKeyResponse {
  ...
  const WebApiKeyResponse({required this.status, required this.data});
