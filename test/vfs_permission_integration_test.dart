...
void main() {
  group('VFS Permission System Tests', () {
    ...
    test('用户创建的文件应该有默认用户权限', () async {
      ...
      expect(permissions.userPermissions & VfsPermission.read, VfsPermission.read);
      expect(permissions.userPermissions & VfsPermission.write, VfsPermission.write);
    ...
    test('权限设置和获取应该正常工作', () async {
      ...
      // 验证权限设置
      final retrievedPermissions = await permissionManager.getPermissions(testPath);
    ...
    test('权限检查方法应该正常工作', () async {
      ...
      // 设置只读权限
      await permissionManager.setPermissions(testPath, VfsPermissionMask(
    ...
    test('系统保护文件不能删除', () async {
      // 尝试删除系统保护文件应该被拒绝
      expect(await permissionManager.canDelete('indexeddb://r6box/fs/.initialized'), false);
      expect(await permissionManager.canDelete('indexeddb://r6box/fs/mnt/'), false);
    });
    ...
    test('权限继承应该正常工作', () async {
      ...
      // 设置父目录权限
      await permissionManager.setPermissions(parentPath, VfsPermissionMask(
        userPermissions: VfsPermission.read,
      ...
      // 创建子文件并应用继承
      final childPath = 'indexeddb://r6box/test_permissions/parent_dir/child_file.txt';
      ...
      // 检查继承的权限
      final childPermissions = await permissionManager.getPermissions(childPath);
    ...
    test('权限过滤应该正常工作', () async {
      ...
      // 设置权限
      await permissionManager.setPermissions(file1, VfsPermissionMask.defaultUser);
      await permissionManager.setPermissions(file2, VfsPermissionMask(
      ...
      // 获取所有文件
      final allFiles = await vfs.listDirectory('indexeddb://r6box/test_permissions');
