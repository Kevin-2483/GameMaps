...
class BuildConfigGenerator {
  static Future<void> main(List<String> args) async {
    ...
    try {
      ...
      if (platformConfigs == null || !platformConfigs.containsKey(targetPlatform)) {
        print('Error: Platform $targetPlatform not found in configuration');
        exit(1);
      }

      final platformConfig = platformConfigs[targetPlatform] as Map<String, dynamic>;
      ...
      buildParams.add('--dart-define=ENABLE_EXPERIMENTAL_FEATURES=${features.contains('ExperimentalFeatures')}');
      buildParams.add('--dart-define=ENABLE_TRAY_NAVIGATION=${features.contains('TrayNavigation')}');

      // 添加构建信息
      final buildConfig = config['build'] as Map<String, dynamic>? ?? {};
      buildParams.add('--dart-define=APP_NAME=${buildConfig['appName'] ?? 'R6Box'}');
      buildParams.add('--dart-define=APP_VERSION=${buildConfig['version'] ?? '1.0.0'}');
      buildParams.add('--dart-define=BUILD_NUMBER=${buildConfig['buildNumber'] ?? '1'}');
